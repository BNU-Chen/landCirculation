 ...
 * @see cn.ac.irsa.landcirculation.Status
...
public class StatusHome {
	...
	public Status findById(java.lang.Integer id) {
		...
		try {
			Status instance = (Status) sessionFactory.getCurrentSession().get("cn.ac.irsa.landcirculation.Status", id);
			if (instance == null) {
	...
	public List findByExample(Status instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("cn.ac.irsa.landcirculation.Status").add(Example.create(instance)).list();
