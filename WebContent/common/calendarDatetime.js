...
// 翻年、月等的按钮
var s_tiannet_turn = "width:25px;" + s_tiannet_turn_base;
// 关闭、清空等按钮样式
var s_tiannet_turn2 = "width:10px;" + s_tiannet_turn_base;
// 年选择下拉框
var s_tiannet_select = "width:64px;display:none;";
// 月、时、分选择下拉框
var s_tiannet_select2 = "width:56px;display:none;";
// 日期选择控件体的样式
...
// 链接的样式
var s_tiannet_link = "text-decoration:none;font-size:9pt;color:#FFF;";
// 横线
var s_tiannet_line = "border-bottom:1 solid #F00";
// ------------------ 变量定义 ---------------------------//
...
document
		.write('<div align="right"><a href="javascript:tiannetClear();" style="'
				+ s_tiannet_link
				+ '">清空</a>'
				...
				+ s_tiannet_link + '">确定</a>&nbsp;</div>');
document
		.write('<div align="center" id="divTiannetDateText" Author="tiannet" style="padding-top:2px;">');
document.write('<span id="tiannetYearHead" Author="tiannet" style="'
		+ s_tiannet_font + '" ' + 'onclick="spanYearCEvent();">&nbsp;年</span>');
document
...
document.write('<input type="button" style="' + s_tiannet_turn2
		+ '" value="↑" title="上一年" onClick="tiannetPrevYear();">');
document.write('<input type="button" style="' + s_tiannet_turn2
		+ '" value="↓" title="下一年" onClick="tiannetNextYear();">&nbsp;');
document.write('<span id="tiannetMonthHead" Author="tiannet" style="'
		+ s_tiannet_font + '" '
...
document.write('<input type="button" style="' + s_tiannet_turn2
		+ '" value="↑" title="上一月" onClick="tiannetPrevMonth();">');
document.write('<input type="button" style="' + s_tiannet_turn2
		+ '" value="↓" title="下一月" onClick="tiannetNextMonth();">');
// document.write('</div>');
...
for ( var i = 0; i < weekName.length; i++) {
	// 输出星期
	document.write('<td width="26" align="center" Author="tiannet">'
			+ weekName[i] + '</td>');
}
document.write(' </tr>');
...
document
		.write('<td align="center" id="tdTiannetDay35" onClick="tiannetDay=this.innerText;tiannetSetValue(true);" '
				+ ' style="' + s_tiannet_day + '">时</td>');
document
		.write('<td align="center" id="tdTiannetDay36" onClick="tiannetDay=this.innerText;tiannetSetValue(true);" '
				+ ' style="' + s_tiannet_day + '">&nbsp;</td>');
document.write('<td align="right" colspan="5">');
document.write('<span id="tiannetHourHead" Author="tiannet" style="'
...
document.write('</select>');
document.write('<span id="tiannetMinuteHead" Author="tiannet" style="'
		+ s_tiannet_font + 'display:none;" '
...
function tiannetWriteHead() {
	...
	if (m_bolShowHour)
		document.all.tiannetHourHead.innerText = "时刻：" + tiannetHour + " 时";
	if (m_bolShowMinute)
...
function tiannetSetValue(bolHideControl) {
	...
	if (m_bolShowMinute) {
		...
		if (minute < 10 && minute.toString().length == 1)
			minute = "0" + minute;
		value += tiannetTimeSplit + minute + tiannetTimeSplit + "00";
	}
	tiannetOutObject.value = value;
...
function tiannetPopCalendar() {
	...
	var h = document.all.divTiannetDate.clientHeight;
	var w = document.all.divTiannetDate.clientWidth;
	// alert("document.all.divTiannetDate.clientHeight:"
	// + document.all.divTiannetDate.clientHeight
	// + "\n+document.all.divTiannetDate.clientWidth:"
	// + document.all.divTiannetDate.clientWidth);
...
function tiannetHideObject(strTagName) {
	...
	for ( var i = 0; i < document.all.tags(strTagName).length; i++) {
		...
		// alert("控件左端:" + x + "select左端" + (objLeft + objWidth) + "控件底部:" +
		// (y+h) + "select高:" + objTop);
